...
def genWind(v_w, end_time, time_step):
    ...
    seed1 = random.randint(-2147483648, 2147483647)
    seed2 = random.randint(-2147483648, 2147483647)
    seed = [seed1, seed2]
...
def pierson_moskowitz_spectrum(U10, zeta, eta, t, random_phases):
    """
    ...
    f_pm = 0.14*(g/U10)  # peak frequency
...
def main(end_time, v_w, time_step = 0.01, Cp_type = 0):
    ...
    t, x, v_wind, wave_eta = rk4(Betti, x0, start_time, end_time, time_step, 0.4, 43000, Cp_type, performance, v_w)
...
def plot_quantiles(results, end_time):
    ...
    for i in range(7):
        ...
        plt.fill_between(t, percentile_37_5[:, i], percentile_62_5[:, i], color='b', alpha=0.65)
...
if __name__ == '__main__':
    ...
    for i in range(n_simulations):
        v_w = genWind(params[1], params[0], 0.01)
        vWind_parallel.append(v_w)
    '''
